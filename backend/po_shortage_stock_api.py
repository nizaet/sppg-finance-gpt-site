...
class StockReferenceRequirementIn(BaseModel):
...
class StockReferenceRequestIn(BaseModel):
    site: Literal["MAJA", "CEMPLANG"]
    requirements: list[StockReferenceRequirementIn] = Field(min_length=1, max_length=50)
...
@router.post("/po-reminders/stock-references")
def po_shortage_stock_references(payload: StockReferenceRequestIn) -> dict[str, Any]:
    """Load same-kitchen stock references only when the operator opens the dialog.

    The reminder already deducted projected local stock. Keeping this read
    separate avoids repeating the expensive inventory projection on every queue
    refresh. Similar names are references only and never close a PO reminder.
    """
    require_db()
