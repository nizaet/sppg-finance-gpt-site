...
def _bank_reference(block: str, fallback_year: str | None = None) -> str | None:
    """Return a wrapped bank beneficiary reference in its canonical form.

    Bank PDFs frequently wrap both the Roman month and the year (for example
    ...
    if fallback_year and re.fullmatch(r"20\d{2}", fallback_year):
        return prefix + fallback_year
    year_match = re.search(r"(?<![-\d])(20\s*\d{2})(?!\d)", block[match.end():])
    return prefix + re.sub(r"\s+", "", year_match.group(1)) if year_match else prefix
...
def _mandiri_status_transactions(text: str) -> list[dict[str, Any]]:
    ...
    if "TRANSACTION STATUS" not in text.upper() or "Reference No." not in text:
        return []
    # The Jasper/BNI export starts each row with date + Reference No.  Some
    # variants insert a time between them, while the PDF uploaded on 13 Sep
    # does not.  Time therefore must be optional, otherwise every row falls
    # through to the AI fallback and only a sampled/incorrect amount remains.
    row_start = r"(?=\b\d{2}-[A-Za-z]{3}-20\d{2}(?:\s+\d{2}:\d{2}:\d{2})?\s+20\d{12,})"
    rows: list[dict[str, Any]] = []
    for block in re.split(row_start, text):
        compact = re.sub(r"\s+", " ", block).strip()
        head = re.match(
            r"(\d{2})-([A-Za-z]{3})-(20\d{2})(?:\s+\d{2}:\d{2}:\d{2})?\s+(20\d{12,})",
            compact,
        )
        if not head:
            continue
        ...
        rows.append({
            ...
            "reference_number": _bank_reference(block, head.group(3)),
            "beneficiary": None,
            "amount": amount,
            "status": status,
            "transaction_date": transaction_date,
