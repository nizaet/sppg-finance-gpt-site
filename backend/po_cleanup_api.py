...
def delete_cancelled_or_test_purchase_order(purchase_order_id: int) -> dict[str, Any]:
    """Permanently remove only a draft, cancelled PO, or an explicitly TEST-* PO.

    Production POs that already have receiving/invoice evidence remain protected.
    TEST-* POs may also clean their test receiving + stock movements, but are
    still blocked once a vendor invoice/payable exists so financial evidence is
    ...
    with connection() as conn:
        with conn.cursor() as cur:
            ...
            if status not in {"DRAFT", "CANCELLED"} and not is_test:
                raise HTTPException(
                    409,
                    "Hanya PO DRAFT, CANCELLED, atau PO berkode TEST-* yang dapat dihapus permanen",
                )
            ...
            if receipt_ids and not is_test:
                raise HTTPException(
                    409,
                    "PO sudah memiliki penerimaan. Hanya PO TEST-* yang boleh membersihkan data penerimaan test.",
                )
