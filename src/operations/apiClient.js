...
export const operationsApi = {
  ...
  correctVendorPayable: (id, payload) => request(`/v1/vendor-payables/${encodeURIComponent(id)}/correct`, { method: "POST", body: JSON.stringify(payload) }),
  cancelVendorPayable: (id, reason) => request(`/v1/vendor-payables/${encodeURIComponent(id)}/cancel`, { method: "POST", body: JSON.stringify({ reason }) }),
  ...
  getInventoryBalances: ({ site, search = "", limit = 300, forDate = "", cookingDate = "" }) => { const q = new URLSearchParams({ site, limit: String(limit) }); if (search) q.set("search", search); if (forDate) q.set("forDate", forDate); if (cookingDate) q.set("cookingDate", cookingDate); return request(`/v1/inventory/balances-v2?${q}`); },
