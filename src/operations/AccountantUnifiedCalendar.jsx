...
import { CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, ExternalLink, FileSearch, RefreshCw, Stamp, Upload, X } from "lucide-react";
...
export default function AccountantUnifiedCalendar({ refreshToken = 0, onChanged, reportError, reportMessage }) {
  ...
  const grouped = useMemo(() => items.reduce((map, row) => {
    const key = dateKey(row.invoice_date);
    if (key) (map[key] ||= []).push(row);
    return map;
  }, {}), [items]);
  const cells = useMemo(() => monthCells(month), [month]);
  const counts = useMemo(() => items.reduce((result, row) => {
    result[stateOf(row)] += 1;
    return result;
  }, { INVOICE: 0, PENDING: 0, APPROVED: 0, PAID: 0 }), [items]);
