    ...
    transform(code, id) {
      ...
      if (id.includes("/src/operations/OperationsAccountantBgn.jsx") && !code.includes("ACCOUNTANT_UNIFIED_CALENDAR_NATIVE")) {
        ...
        out = out.replace(
          ...
          'const existingMaker = x.maker_id ? { maker_id:x.maker_id, maker_status:x.maker_status } : (x.invoice_id != null ? makerByInvoice.get(String(x.invoice_id)) : null);'
        );
      ...
      if (id.includes("/src/operations/OperationsPayments.jsx")) {
        ...
        if (!out.includes('paymentModalItem&&<VendorPaymentEvidenceModal')) {
          out = out.replace(
            '<div className="ops-domain-stack">',
            '<div className="ops-domain-stack">{paymentModalItem&&<VendorPaymentEvidenceModal item={paymentModalItem} onClose={()=>setPaymentModalItem(null)} onSaved={async(result)=>{setPaymentModalItem(null);setActionMessage(`Pembayaran tersimpan. Status tagihan: ${result.payableStatusAfter||result.reconciliationStatus||"PAID"}. Bukti sudah masuk Google Drive.`);await load();}}/>}'
          );
        }
        ...
        out = replaceAfter(out, invoiceMarker, '<td>{item.payable_status || "UNPAID"}</td>\n                </tr>', '<td><strong>{CLOSED.has(String(item.payable_status||"UNPAID").toUpperCase())?"✓ PAID":(item.payable_status||"UNPAID")}</strong></td><td>{CLOSED.has(String(item.payable_status||"UNPAID").toUpperCase())?<span style={{color:"#166534",fontWeight:800}}>✓ Selesai</span>:<button type="button" onClick={()=>setPaymentModalItem(item)}>Bayar</button>}</td>\n                </tr>');
