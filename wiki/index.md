# SPPG LLM Wiki Index

## Sites
- [SPPG Maja](sites/maja.md)
- [SPPG Cemplang](sites/cemplang.md)

## Procurement
- [Vendor Map](procurement/vendor-map.md)
- [Lead Time](procurement/lead-time.md)
- [Vendor Master Rules](procurement/vendor-master.md)

## Accounting
- [Costing](accounting/costing.md)
- [Accountant Workflow](accounting/accountant-workflow.md)
- [Internal Cash Reimbursement](accounting/internal-reimbursement.md)
- [BGN Workflow](accounting/bgn-workflow.md)

## Inventory
- [Koperasi Dry-Goods Flow](inventory/koperasi-flow.md)
- [PO Reminder and Kitchen Warehouse Scope](inventory/po-reminder-stock-scope.md) — exact PO coverage, selected-kitchen stock, on-demand references, and reminder cache rules.

## People
- [People & Roles](people/index.md)

## Suppliers
- [Supplier Master](suppliers/index.md)

## Evidence Storage
- [Google Drive Evidence Archive](../operations/drive_archive_v01.md)

## Structured Data
- `data/master_entities.json`
- `parser/whatsapp_event_schema.json`

## Audit / Change Log
- [Knowledge Log](log.md)
