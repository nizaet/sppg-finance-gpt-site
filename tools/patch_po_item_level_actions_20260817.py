...
def replace_from(path: str, start: str, replacement: str) -> None:
    p = Path(path)
    text = p.read_text(encoding="utf-8")
    a = text.find(start)
    if a < 0:
        raise SystemExit(f"{path}: start anchor not found: {start!r}")
    p.write_text(text[:a] + replacement, encoding="utf-8")


# v4 exposes exact item-level PO coverage facts already calculated by the strict engine.
replace_once(
...
# UI actions and item-level labels.
replace_once(
